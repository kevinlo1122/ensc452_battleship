...
//			}
			...
			// process p2 attack
//			coord received_coords;
//			received_coords.x = recv_x;
//			received_coords.y = recv_y;
//			attackPos(p1_ships, received_coords, '2');
...
void setupP1ShipPlacement_old(ship* ships)
{
...
void setupShips(ship* ships)
{
	...
	for(int i = 0; i < NUM_SHIPS; i++){
		...
		for(int j = 0; j < NUM_COORDS; j++){
			if (j < types[i])
			{
				ships[i].coords[j].x = j;
				ships[i].coords[j].y = i;
			}
			else
			{
...
void attackPos_old(ship* ships, coord coord)
{
